...
public class ComSourceEmitterTests
{
    ...
    public void Checked_in_ir_and_host_sources_match_generator()
    {
        ...
        Assert.Equal(
            Normalize(ir.ToJson()),
            Normalize(File.ReadAllText(Path.Combine(root, "rustolonia", "rust", "projection.ir.json"))));

        var generatedDirectory = Path.Combine(root, "rustolonia", "host", "Generated", "ObjectModel");
