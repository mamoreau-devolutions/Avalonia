...
type PopupPlacementCallback = Box<
    dyn FnMut(f64, f64, f64, f64, f64, f64) -> Result<PopupPlacementResult> + Send,
>;
...
unsafe extern "system" fn popup_placement_invoke(
    ...
    if offset_x.is_null() || offset_y.is_null() || anchor.is_null()
        || gravity.is_null() || constraint_adjustment.is_null()
    {
        return hresult::E_POINTER;
    }
    let object = this.cast::<PopupPlacementObject>();
    let mut arguments = (popup_width, popup_height, anchor_x, anchor_y, anchor_width, anchor_height);
