...
fn host_path() -> PathBuf {
    ...
    let candidates = [
        "rustolonia/host/bin/Release/net10.0/win-x64/publish/Avalonia.Host.dll",
        "rustolonia/host/bin/Release/net10.0/win-arm64/publish/Avalonia.Host.dll",
        "rustolonia/host/bin/Debug/net10.0/win-x64/publish/Avalonia.Host.dll",
    ];
    #[cfg(target_os = "linux")]
    let candidates = [
    ...
    panic!(
        "Avalonia.Host native library not found. Run `rustolonia/rust/build.ps1` on \
         Windows or `rustolonia/rust/build.sh` on Linux, or set AVN_HOST_NATIVE_LIB."
    );
}
