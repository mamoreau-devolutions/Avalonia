...
```bash
pwsh ./rustolonia/rust/new-app.ps1 my_app ~/src/my_app
```
...
```bash
pwsh ./rustolonia/rust/regenerate-and-build.ps1
```
...
| `-Test` | `AVN_RUN_CARGO_TESTS=1` | Run `cargo test --workspace` instead of `cargo build --workspace`. Requires a host discoverable per [Host discovery](#host-discovery) below (`rustolonia/rust/build.ps1` publish one; set `AVN_HOST_NATIVE_LIB` otherwise). |
...
This script intentionally does not replace `rustolonia/rust/build.ps1`
...
1. **`AVN_HOST_NATIVE_LIB`** (the `avalonia::HOST_NATIVE_LIB_ENV_VAR` constant) -- an explicit
   ...
   how `rustolonia/rust/build.ps1` point the workspace test suite at a
...
`rustolonia/rust/avalonia/src/runtime.rs` (`discover_host_path`, `HOST_NATIVE_LIB_ENV_VAR`)
for the implementation, `rustolonia/rust/avalonia/src/runtime.rs`'s
`host_discovery_tests` module for unit tests of the override/adjacent-lookup
precedence and error message, and `rustolonia/rust/avalonia/tests/host_discovery.rs` for
...
regardless of platform, under `rustolonia/rust/artifacts/<rid>/`:
...
```bash
pwsh ./rustolonia/rust/package.ps1 linux-x64
```

```bash
pwsh ./rustolonia/rust/package.ps1 osx-arm64
```
...
- **The `rustolonia/rust/*` crates** are source only (`publish = false`, see above) and
...
- **The `rustolonia/rust/package.ps1` output** (`Avalonia.Host` plus
...
- **Stage 29 desktop file integration** adds only source files to the existing
  non-packable `Avalonia.Host` and to the source-only `rustolonia/rust/*` crates, plus
  ...
  nor `rustolonia/rust/generate-sbom.ps1`'s delivery inventory changes.

## Tests

- `rustolonia/rust/avalonia/src/runtime.rs` (`host_discovery_tests` module) and
  `rustolonia/rust/avalonia/tests/host_discovery.rs` cover `discover_host_path`: the
...
- `regenerate-and-build.ps1 -ValidateTemplate` / `AVN_VALIDATE_TEMPLATE=1
  ...
  rustolonia/rust/templates/avalonia-app/Cargo.toml` compile-check the application
