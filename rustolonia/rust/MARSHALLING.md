...
`rustolonia/rust/regenerate-and-build.ps1`:

- `rustolonia/projection/Avalonia.Projection.Ir/GeometryMarshalling.cs` is the managed source of
  truth (kind, CLR type, ABI name, field list, conversion style).
- `rustolonia/projection/Avalonia.Projection.Ir/BrushMarshalling.cs` is the equivalent for the
  solid brush interface (CLR types, ABI interface name, factory method name).
- `ComSourceEmitter` writes the C# structs into
  `rustolonia/host/Generated/ObjectModel/ProjectionStructs.g.cs`, each with
...
- `NativeHeaderEmitter` writes the `typedef struct Avn*` declarations into
  `rustolonia/rust/avalonia-sys/include/avalonia-rust-abi.h`.
- `rustolonia/rust/avalonia-bindgen/src/geometry.rs` is the Rust-side source of truth; it
